...
from models import User, Habit, Achievement, Group, Competition
...
@app.post("/users/")
def create_user(name: int, tg_id: str, habits: dict, achievements: dict, groups: dict, competitions: int, db: Session = Depends(get_db)):
    user = User(name=name, tg_id=tg_id, habits=habits, achievements=achievements, groups=groups, competitions=competitions)
    db.add(user)
    db.commit()
    db.refresh(user)
    return user
